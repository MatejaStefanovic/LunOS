...
void terminal_render(void) {
    for (size_t y = 0; y < total_rows; y++) {
        for (size_t x = 0; x < total_columns; x++) {
            ...
            fb_put_char(ch, x * FONT_WIDTH, y * FONT_HEIGHT,
                    terminal_fg_color, terminal_bg_color);
        }
    }
}
...
void terminal_putentryat(char c, size_t x, size_t y) {
    ...
    fb_put_char(c, x * FONT_WIDTH, y * FONT_HEIGHT,
                terminal_fg_color, terminal_bg_color);
...
void terminal_setcolor(uint32_t fg, uint32_t bg) {
