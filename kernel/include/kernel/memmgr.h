...
int mm_setup_executable(struct mem_descriptor *mm, vaddr_t code_start, 
                       vaddr_t code_end, vaddr_t data_end);

// brk() is for heap 
vaddr_t mm_brk(struct mem_descriptor *mm, vaddr_t new_brk);

struct mem_region* mm_find_region(struct mem_descriptor *mm, vaddr_t addr);
...
// for mmap()
int mm_munmap(struct mem_descriptor *mm, vaddr_t addr, size_t len);
vaddr_t mm_mmap(struct mem_descriptor *mm, vaddr_t addr, size_t len, 
                                                    int prot, int flags);

bool mm_check_access(struct mem_descriptor *mm, vaddr_t addr, uint64_t flags);
