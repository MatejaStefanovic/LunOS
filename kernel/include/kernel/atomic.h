...
static inline void atomic64_init(atomic64_t *v, long val){
    v->value = val;
}
...
static inline void atomic64_set(atomic64_t *v, long val){
    v->value = val;
}
...
static inline void atomic64_add(long val, atomic64_t *v){
...
static inline void atomic64_sub(long val, atomic64_t *v){
...
{
    char result;
    __asm__ __volatile__("lock incl %0; setz %1"
                         : "+m" (v->value), "=qm" (result)
                         :
                         : "memory");
    return result;
...
static inline int atomic_try_cmpxchg(atomic_t *v, int *old_val, int new_val)
{
    char success;
    int prev = *old_val;
    __asm__ __volatile__("lock cmpxchgl %3, %1; setz %0"
...
static inline int atomic64_try_cmpxchg(atomic64_t *v, long *old_val, long new_val)
{
    char success;
    long prev = *old_val;
    __asm__ __volatile__("lock cmpxchgq %3, %1; setz %0"
                         : "=a" (success), "+m" (v->value), "+a" (prev)
                         : "r" (new_val)
                         : "memory");
    *old_val = prev;
...
{
    char old_bit;
    __asm__ __volatile__("lock btsq %2, %0; setc %1"
                         : "+m" (*addr), "=qm" (old_bit)
                         : "Ir" (bit)
                         : "memory");
    return old_bit;
...
{
    char old_bit;
    __asm__ __volatile__("lock btrq %2, %0; setc %1"
                         : "+m" (*addr), "=qm" (old_bit)
                         : "Ir" (bit)
                         : "memory");
    return old_bit;
...
{
    char old_bit;
    __asm__ __volatile__("lock btcq %2, %0; setc %1"
                         : "+m" (*addr), "=qm" (old_bit)
                         : "Ir" (bit)
                         : "memory");
    return old_bit;
